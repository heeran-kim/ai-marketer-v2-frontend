...
const ListCard = forwardRef<HTMLDivElement, ListCardProps>(({ item, actions }, ref) => {
    const cardRef = useRef<HTMLDivElement>(null);
    const [isMobileLayout, setIsMobileLayout] = useState(false);

    const [imagePreviewUrl, setImagePreviewUrl] = useState<string>("/media/no-post.jpg");
    const [scheduleType, setScheduleType] = useState<"instant" | "scheduled">("instant");
    const [date, setDate] = useState<string>("");
    const [socialLinks, setSocialLinks] = useState<{link: string, platform: string}[]>([]);
    ...
    useEffect(() => {
        ...
        if ((item as Post).type === "post") {
            ...
            setDate(format(new Date((item as Post).scheduledAt), "yyyy-MM-dd hh:mm a"));
            setSocialLinks([{ link: (item as Post).link ?? "Link not available yet", platform: (item as Post).platform }]);
            setDescription((item as Post).caption);
            setStatus((item as Post).status);
        }
        
        if ((item as Promotion).type === "promotion") {
            setImagePreviewUrl(((item as Promotion).posts ?? []).length > 0 ? (item as Promotion).posts[0].image : imagePreviewUrl);
            setDate(`${format(new Date((item as Promotion).startDate), "yyyy-MM-dd")} ~ ${format(new Date((item as Promotion).endDate), "yyyy-MM-dd")}`);
            setSocialLinks((item as Promotion).posts?.map((post) => ({
    ...
    return (
        <div
            ...
            className={`relative p-4 bg-white rounded-lg shadow-md border transition hover:shadow-lg flex 
                ${isMobileLayout ? "flex-col items-center" : "flex-row items-start space-x-4"}`}
        >
            {actions && (
                        ...
                        {item.type === "postReview" ? (
                            <div className="flex flex-col w-full">
                                <select 
                                    value={scheduleType} 
                                    onChange={(e) => {
                                        if (e.target.value === "instant") setDate(new Date().toISOString().slice(0,16));
                                        setScheduleType(e.target.value as "instant" | "scheduled")
                                    }}
