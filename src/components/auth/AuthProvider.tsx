...
interface AuthContextType {
    ...
    unlinkSocialAccount: (provider: string) => Promise<void>; // Unlink social account
    deleteAccount: () => Promise<void>; // Delete account
}
...
export const AuthProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const register = async (name: string, email: string, password: string) => {
     ...
     * Removes a linked social auth provider from the user account
     */
    const unlinkSocialAccount = async (provider: string) => {
        await fetchWithAuth(USERS_API.UNLINK_SOCIAL, "POST", { provider });
    ...
    const deleteAccount = async () => {
        await fetchWithAuth(USERS_API.DELETE_ACCOUNT, "DELETE");
        setAuthState({ status: "unauthenticated" });
