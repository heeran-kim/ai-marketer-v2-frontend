    ...
    return (
        <DndProvider backend={HTML5Backend}>
            <div className="flex flex-col h-full">
                {/* Selected Captions */}
                <div className={`transition-all duration-300 bg-gray-100 rounded-lg ${selectedView === "suggestedCaptions" ? "h-[calc(100%-70px)]" : "h-[40px]"}`}>
                    <div
                            ...
                            >
                                {captionSuggestions.length > 0 ? (
                                    captionSuggestions.map((caption, index) => (
                                        <SwiperSlide key={index} className="w-auto flex item-stretch h-full">
                                            <DraggableCaption
                                                id={caption}
                                                text={caption}
                                                index={index}
                ...
                {/* Platform Selection */}
                <div className={`transition-all duration-300 ${selectedView !== "suggestedCaptions" ? "h-[calc(100%-40px)]" : "h-[40px]"}`}>
                    ...
                    {selectedView !== "suggestedCaptions" && (
                        <div className="p-3" style={{ height: "calc(100% - 70px)" }}>
                            {platformStates
                                ...
                                .map((platform) => (
                                    <div key={platform.key} className="p-3 border rounded-md h-full">
